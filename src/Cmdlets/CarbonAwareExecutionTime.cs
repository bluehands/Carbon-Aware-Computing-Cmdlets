...
public class CarbonAwareExecutionTimeCmdlet : CmdletBase
{
...
[Cmdlet(Verbs.Get, Nouns.GridCarbonIntensity)]
public class GridCarbonIntensityCmdlet : CmdletBase
{
    [Parameter(Mandatory = true)]
    public string Location { get; set; }
    [Parameter(Mandatory = true)]
    ...
    protected override void ProcessRecord()
    {
        base.ProcessRecord();
        var processor = Provider == DataProvider.OpenData ?
            new GridCarbonIntensityProcessor() :
            new GridCarbonIntensityProcessor(new WattTimeCredentials(WattTimeUsername, ConvertFromSecureString(WattTimePassword)));
        processor.Process(Location).GetAwaiter().GetResult()
            .Switch(
                e => WriteObject(e.Value),
                e => HandleError("No data available", default)
            );
    }
